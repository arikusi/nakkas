...
* Hardening findings from the pre-release edge-case audit (see `dogfooding.md`):
  * `orient: "auto-start-reverse"` now produces a design note: pixel-diffing resvg against Chromium showed browsers flip the start marker while the preview renders it unflipped, so the response says so instead of letting the preview lie silently.
  * A line, polyline or path carrying markers but no stroke produces a design note: the shape itself is invisible and the markers stay at their smallest size, which is almost never the intent.
  * Invisible text (opacity 0, fillOpacity 0 or visibility hidden) no longer participates in the text layout audit; a hidden helper text overlapping a visible one was a false positive.
  * The bbox-based audits stopped paying resvg's ~90ms system-font scan per isolated render: when a config sticks to generic font families the three resolved font files are passed directly. The 24-text worst case dropped from ~2.3s to ~25ms per render with measurements identical to a tenth of a pixel; custom font names keep the full scan for true metrics.
* The browser ground-truth check is now an automated test: 12 seeded random translateX animations (named easings and random cubic-beziers, multi-stop keyframes, random durations and sample times) are frozen in headless Chromium via the negative animation-delay trick and compared against bakeFrame's output, with a 1px tolerance. Deterministic seed in CI, `BROWSER_TRUTH_SEED` to fuzz new cases; skips with a warning when no Chromium binary exists. Verified by mutation: a planted 10% easing error fails the test, reverting restores green.
* 397 tests (9 new covering marker rendering, reference normalization, size/orient, refcheck and the gradient contrast path, plus the browser cross-check).
